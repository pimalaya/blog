...
- SEO: JSON-LD structured data on every page (`Blog` on the index, `BlogPosting` with publish date on posts), `og:type: article` + `article:published_time` on posts, a 1200×630 social card (og.png, from the logo and theme palette) wired to `og:image` and `twitter:card`, and `<lastmod>` dates in the sitemap.
